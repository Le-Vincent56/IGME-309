...
void MyMesh::computeAABB() {
	// Initialize minVert and maxVert
	minVert = vec3(vertices[0], vertices[1], vertices[2]);
	maxVert = vec3(vertices[0], vertices[1], vertices[2]);

	for (unsigned int i = 0; i < vertNum; i++) {
		// Retrieve the vertex
		vec3 vertex(vertices[i * 3 + 0], vertices[i * 3 + 1], vertices[i * 3 + 2]);
	...
	std::cout << "Max Vert: (" << maxVert.x << ", " << maxVert.y << ", " << maxVert.z << ")";
	std::cout << "Min Vert: (" << minVert.x << ", " << minVert.y << ", " << minVert.z << ")";
}
