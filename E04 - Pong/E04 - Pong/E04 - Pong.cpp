...
void update() {
	...
	if (ballIsActive == false) {
		...
		// Calculate y-distance from the ball to the bar
		float distanceY = ballPos[1] - barPos[1];

		// Check if the y distance is less than the radius, if so, then the circle has hit the correct y-level
		bool distY = distanceY < radius;

		// Check if the circle is within the bounds of the bar
		bool leftBound = (ballPos[0] + radius) >= (barPos[0] - (barSize/2));
		bool rightBound = (ballPos[0] - radius) <= (barPos[0] + (barSize/2));
		
		// If the y-distance is less than the radius, and the circle is within the x-bounds, then a collision has occured
		if (distanceY < radius && leftBound && rightBound) {
			// Invert the positions to mimic a bounce
			velocity_x *= -1;
			velocity_y *= -1;
		}
