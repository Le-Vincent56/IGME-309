...
int GenerateRandomNumber() {
	...
	// Get a number between 1 and 100, inclusive
...
int PlayerEnterANumber() {
	...
	// Get the console input and assign it to playerNumber
	cin >> playerNumber;
...
int HighOrLow(int targetNumber, int playerNumber) {
	...
	if (playerNumber > targetNumber) {
		// If larger than target, return 2
		return 2;
	}
	else if (playerNumber < targetNumber) {
		// If smaller than target, return 1
		return 1;
	}
	else if (playerNumber == targetNumber) {
...
void GameLoop() {
	...
	while (g_is_running) {
		...
		// Put a space in between for cleaner output
		cout << endl;
	}
